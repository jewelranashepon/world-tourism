...
export default async function BlogPage() {
  ...
  const continents = Array.from(
    new Set(blogPosts.map((post) => post.continent).filter(Boolean) as string[])
  ).map((name) => ({
    name,
      ...
      <div className="flex flex-col items-center justify-center space-y-4 text-center">
        <div className="space-y-2">
          ...
          <p className="max-w-[900px] text-gray-500 md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed dark:text-gray-400">
            Discover travel tips, stories, and inspiration from around the
            world.
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="all" className="mt-12">
        <TabsList className="mb-10 flex flex-wrap justify-center gap-3 bg-transparent p-0 shadow-none border-none">
          ...
          {continents.map((continent) => (
            <TabsTrigger key={continent.slug} value={continent.slug} className="px-5 py-2 text-sm font-medium rounded-full transition-colors duration-200 border border-gray-300 dark:border-gray-700 data-[state=active]:bg-blue-600 data-[state=active]:text-white hover:bg-blue-100 dark:hover:bg-blue-800">
              {continent.name}
            </TabsTrigger>
          ))}
        </TabsList>
